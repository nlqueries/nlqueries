...
Note that `nlqueries-core` stores database credentials and API keys locally (`~/.nlqueries/connectors.yaml` and environment variables) and never transmits them anywhere except the target database or configured LLM provider. If you find a case where that's not true, that's exactly the kind of issue this policy is for.
