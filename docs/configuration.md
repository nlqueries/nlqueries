...
| `LLM_MAX_OUTPUT_TOKENS` | No | `1024` | Tokens an answer may generate. Every other budget derives from it: a SQL correction or candidate set gets half (floor 512), intent classification and follow-up resolution an eighth (floor 200). **Raise it for a reasoning model** — such a model bills its private reasoning from the same allowance and spends it first, so a budget sized for the answer alone returns a short answer, or an empty one on the smallest calls. Clamped to at least 1. |
