...
| `LLM_MAX_OUTPUT_TOKENS` | No | `1024` | Tokens an answer may generate, and the two derived budgets: a SQL correction or candidate set gets half (floor 512), intent classification and follow-up resolution an eighth (floor 200) — note that the floor binds until this exceeds **1600**, so raising it from 1024 to, say, 1536 moves the answer and correction budgets but leaves classification at 200. It does **not** govern knowledge-base column descriptions, which size their own allowance from the column count (`kb_generator`), nor the CLI and MCP health checks. **Raise it for a reasoning model** — such a model bills its private reasoning from the same allowance and spends it first, so a budget sized for the answer alone returns a short answer, or nothing at all on the smallest calls. Producing nothing now raises `OutputBudgetExhausted`, naming the model and the budget it spent, rather than returning an empty string that is indistinguishable from a model with nothing to say. Follow-up resolution and knowledge-base column descriptions are the exception: they log it at `WARNING` and carry on with a degraded result, because an unresolved follow-up is still worth asking. A non-positive value is ignored with a warning and the default applies. |
| `LLM_TIMEOUT_SECONDS` | No | `180` | How long an LLM call may go **without producing data** before it fails. Not a rescue from an unbounded wait: both SDKs already impose 600s of their own (`anthropic`'s `DEFAULT_TIMEOUT`, litellm's `COMPLETION_HTTP_FALLBACK_SECONDS`), so what this buys is 600 → 180, configurable, plus a single `LLMTimeout` to catch in place of one exception type per SDK. **It is a read deadline, not a cap on the call**: httpx applies it per read, so it fires after this many seconds of *silence*. On the streaming paths the Anthropic Messages API sends periodic `ping` events that each reset the clock, so a stream that keeps trickling can outlast it — do not size an upstream budget as though this capped a question. 180 is deliberately generous rather than tight: a reasoning model on a long schema can take a minute or more before its first token, and a deadline that fires on those trades a hang for a broken deployment. Not to be confused with `QUERY_STATEMENT_TIMEOUT_SECONDS`, which is the budget for the SQL, not for the model. A non-positive, non-finite or unparseable value is ignored with a warning — `inf` included, since it parses and would hand the SDKs a deadline that never fires. |
...
| `NLQ_MAX_DOCUMENT_EXPANDED_BYTES` | No | `419430400` (400 MiB) | Largest total **uncompressed** size permitted in a zip-based document (`.xlsx`, `.docx`). An upload is size-limited and a request body is capped, but neither bounds what comes out of a parser, and the size on disk says nothing about it: measured on ordinary content, 200k rows by 10 columns is 5.2 MiB on disk and 108.8 MiB expanded. Read from the zip directory before any parsing, so nothing is decompressed to find out. A malformed value falls back to the default rather than failing the ingest. |
| `NLQ_MAX_DOCUMENT_EXPANSION_RATIO` | No | `100` | Largest expanded-over-compressed ratio permitted in a zip-based document. Complementary to the byte ceiling rather than redundant with it: a small file that expands 1000× is a bomb even when the result fits under the ceiling, and a large file that expands 2× is ordinary. 100× is roughly three times the highest ratio measured on real content. |
| `NLQ_MAX_DOCUMENT_ROWS` | No | `50000` | Most spreadsheet rows extracted from one document, across all its sheets. The expansion limits above read what a zip archive declares about itself; this is counted while reading, so it bounds a document whatever its directory claims. Measured: 200,000 rows by 10 columns took 87 seconds to ingest from a 5.2 MiB file. At the connector's batch size of 50, 50,000 rows is already a thousand chunks from one document. |
| `NLQ_MAX_EXTRACTION_SECONDS` | No | `120` | Wall-clock budget for extracting one document, in every connector. One budget rather than a page cap for PDFs, a paragraph cap for Word and a row cap for spreadsheets — per-format counts are guesses about cost, and a 3,000-page PDF of scanned images and a 30-page one of dense tables differ by orders of magnitude. Checked between units of work (a page, a section, a batch of rows), so it bounds a long document rather than interrupting one slow page. A ceiling on background ingestion, not a latency target. |
