...
| `LLM_TIMEOUT_SECONDS` | No | `180` | How long a single LLM call may take before it fails. Until this existed there was no deadline anywhere: a slow or stuck model hung the request indefinitely — one server log showed a **seven-minute gap** between calls on a single question, and a later question that made two calls and then simply stopped, with no error and no answer. A call that passes the deadline raises `LLMTimeout`, naming the model and the limit. 180 is deliberately generous rather than tight: a reasoning model on a long schema can take a minute or more before its first token, and a timeout that fires on those trades a hang for a broken deployment. It bounds a **call**, not a question — a question that classifies, generates SQL and then corrects it makes three calls. Not to be confused with `QUERY_STATEMENT_TIMEOUT_SECONDS`, which is the budget for the SQL, not for the model. A non-positive or unparseable value is ignored with a warning; there is no value meaning "wait forever", because that was the behaviour this setting exists to end. |
