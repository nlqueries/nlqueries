...
class TestQuery:
    @pytest.mark.anyio
    async def test_answer_always_present(self) -> None:
        from nlqueries.mcp_server.server import query

        result = _make_result(answer="42 orders.", sql=None)
        with patch(
            "nlqueries.orchestrator.sync_runner.run_query",
            new_callable=AsyncMock,
            return_value=result,
        ):
            out = await query("How many orders?", "sales")

        assert "42 orders." in out

    @pytest.mark.anyio
    async def test_sql_block_included_when_present(self) -> None:
        from nlqueries.mcp_server.server import query
    ...
    @pytest.mark.anyio
    async def test_sql_block_absent_when_none(self) -> None:
        from nlqueries.mcp_server.server import query
        ...
        with patch(
            ...
            out = await query("Summarise the docs", "docs_agent")

        assert "```sql" not in out

    @pytest.mark.anyio
    async def test_citations_included_when_present(self) -> None:
        from nlqueries.mcp_server.server import query
        ...
        with patch(
            ...
            out = await query("What drove revenue?", "docs_agent")
    ...
    @pytest.mark.anyio
    async def test_citation_without_page_number(self) -> None:
        from nlqueries.mcp_server.server import query
        ...
        with patch(
            ...
            out = await query("What is X?", "docs_agent")

        assert "wiki.md" in out
        assert "page" not in out

    @pytest.mark.anyio
    async def test_long_excerpt_truncated(self) -> None:
        from nlqueries.mcp_server.server import query
        ...
        with patch(
            ...
            out = await query("Tell me about X", "docs_agent")
    ...
    @pytest.mark.anyio
    async def test_sql_execution_error_shown(self) -> None:
        from nlqueries.mcp_server.server import query
    ...
    @pytest.mark.anyio
    async def test_metadata_footer_present(self) -> None:
        from nlqueries.mcp_server.server import query
    ...
    @pytest.mark.anyio
    async def test_agent_not_found_returns_friendly_message(self, tmp_path: Path) -> None:
        from nlqueries.mcp_server.server import query
        ...
        with (
            ...
            out = await query("How many orders?", "ghost_agent")
    ...
    @pytest.mark.anyio
    async def test_passes_dialect_to_run_query(self) -> None:
        from nlqueries.mcp_server.server import query
        ...
        with patch(
            ...
            await query("How many orders?", "sales", dialect="snowflake")
